...
A [jbr](https://github.com/rubensworks/jbr.js/tree/master/packages/jbr) hook type for a [Comunica](https://github.com/comunica/comunica)-based SPARQL endpoint. This hook allows the endpoint to use a local file as datasource. However, there is no official docker image for comunica that can query local files, so users should make their own docker image of the engine [comunica/query-sparql-file](https://github.com/comunica/comunica/tree/master/engines/query-sparql-file). Docker images for comunica can be created using [lerna-docker](https://github.com/rubensworks/lerna-docker). By default, there is no dockerfile in the query-sparql-file, but it can be copied directly from the engine [comunica/query-sparql](https://github.com/comunica/comunica/tree/master/engines/query-sparql).
