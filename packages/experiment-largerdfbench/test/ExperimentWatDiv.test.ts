...
describe('ExperimentWatDiv', () => {
  ...
  let experiment: ExperimentLargeRdfBench;
  beforeEach(() => {
    context = {
      cwd: 'CWD',
    ...
    experiment = new ExperimentLargeRdfBench(
      1,
      5,
      1,
      true,
  ...
  describe('prepare', () => {
    it('should prepare the experiment', async() => {
      ...
      expect(context.docker.imagePuller.pull).toHaveBeenCalledWith({
        repoTag: ExperimentLargeRdfBench.DOCKER_IMAGE_WATDIV,
      });
      expect(context.docker.imagePuller.pull).toHaveBeenCalledWith({
      ...
      expect(context.docker.containerCreator.start).toHaveBeenCalledWith({
        imageName: ExperimentLargeRdfBench.DOCKER_IMAGE_WATDIV,
        cmdArgs: [ '-s', '1', '-q', '5', '-r', '1' ],
        hostConfig: {
          Binds: [
